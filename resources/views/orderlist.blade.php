                    ...
                    <div class="card border-2" style="background-color: #405D72">
                        <div class="card-body">
                            <div class="title text-center" style="color: #F5F7F8" ;>
                                <img src="{{ asset('assets/cargo.png') }}" alt="User Logo"
                                    style="width: 50px; height: auto;" draggable="false">
            ...
            <table class="table table-bordered mt-2">
                <thead>
                    <tr class="text-center">
                        ...
                        <th>Price</th>
                        <th>Status</th>
                        @if ($orders->whereNotIn('status', ['completed', 'rejected'])->isNotEmpty())
                    ...
                    @else
                        @foreach($orders as $order)
                            <tr class="text-center">
                                ...
                                <!-- Only show the Update Status column if the order is not rejected -->
                                @if($order->status != 'rejected' && $order->status != 'completed')
                                    <td>
                                        <form action="{{ route('updatestatus', $order->id) }}" method="POST"
                                            ...
                                            <!-- Show the select box options based on current status -->
                                ...
                                <td>
                                    <form action="{{ route('deleteorders', $order->id) }}" method="POST"
                                        ...
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" class="btn btn-danger btn-sm" title="Delete Order"
